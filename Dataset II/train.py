...
# 下面是视觉特征
train_visual = "data/visual_feature.npy"
...
with open("data/test_data.csv", 'r') as reader:
    reader.readline()
...
def getTrainBatch(batchSize, num):
    ...
    for i in range(num * batchSize, (num + 1) * batchSize):
        ...
        pos_input[i % batchSize] = list(map(lambda n: n + 1, pos_his_data[user_id][:pos_len])) + [
            len(train_visual) - 1] * (max(timestep, pos_len) - pos_len)
        ...
        neg_input[i % batchSize] = list(map(lambda n: n + 1, neg_his_data[user_id][:neg_len])) + [
            len(train_visual) - 1] * (max(timestep, neg_len) - neg_len)
    ...
    return label, pos_input, pos_mask_input, pos_edge_input.reshape(
        [-1, timestep, 1]), neg_input, neg_mask_input, neg_edge_input.reshape([-1, timestep, 1]), item_input
...
def pos_eva(sess, model):
    ...
    for valNum in tqdm(range(valLoop)):
        label, pos_input, pos_mask_input, pos_edge_input, neg_input, neg_mask_input, neg_edge_input, item_input = getValBatch(
            val_batchsize, valNum)
...
with open(time + ".txt", "a") as f:
    ...
    for epoch in range(100):
        ...
        for num in tqdm(range(num)):
            ...
            label, pos_input, pos_mask_input, pos_edge_input, neg_input, neg_mask_input, neg_edge_input, item_input = getTrainBatch(
                2048, num)
