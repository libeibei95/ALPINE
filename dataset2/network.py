...
class Lstm(object):
    ...
    def __init__(self, W_embedding, settings):
        ...
        with tf.name_scope('Inputs'):
            ...
            self.pos_mask_inputs = tf.placeholder(tf.int32, [None, ],
                                                  name='pos_mask_inputs')  # postive history sequence mask
            self.pos_edge_inputs = tf.placeholder(tf.int32, [None, self.timestep, 1],
                                                  name='pos_edge_inputs')  # postive history edge
        ...
        self.joint_output = 0.68 * self.pos_output + 0.32 * self.neg_output

        self.pos_evaoutput = tf.nn.sigmoid(self.pos_output)
        self.neg_evaoutput = tf.nn.sigmoid(self.neg_output)
        self.joint_evaoutput = tf.nn.sigmoid(self.joint_output)
        with tf.name_scope('pos_loss'):
            l2_norm = tf.add_n([tf.nn.l2_loss(v) for v in tf.trainable_variables() if 'w' and 'pos' in v.name])
            self.pos_loss = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(logits=self.pos_output,
                                                        labels=self.y_inputs)) + l2_norm * 0.00005
        with tf.name_scope('neg_loss'):
            l2_norm = tf.add_n([tf.nn.l2_loss(v) for v in tf.trainable_variables() if 'w' and 'neg' in v.name])
            self.neg_loss = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(logits=self.neg_output,
                                                        labels=self.y_inputs)) + l2_norm * 0.00005
        with tf.name_scope('joint_loss'):
            ...
            self.joint_loss = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(logits=self.joint_output,
                                                        labels=self.y_inputs)) + l2_norm * 0.00005
    ...
    def lstm_cell_forward(self, xt, h_prev, c_prev, h_edge, c_edge):
        with tf.variable_scope('rnn_cell', reuse=True):
            ...
            Wix = tf.get_variable(shape=[self.item_dim, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                  name='Wix')
            Wih = tf.get_variable(shape=[self.hidden_size, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                  name='Wih')
            ...
            # 遗忘门
            Wfx = tf.get_variable(shape=[self.item_dim, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                  name='Wfx')
            Wfh = tf.get_variable(shape=[self.hidden_size, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                  name='Wfh')
            ...
            # 输出门
            Wox = tf.get_variable(shape=[self.item_dim, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                  name='Wox')
            Woh = tf.get_variable(shape=[self.hidden_size, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                  name='Woh')
            ...
            # 当前记忆
            Wmx = tf.get_variable(shape=[self.item_dim, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                  name='Wmx')
            Wmh = tf.get_variable(shape=[self.hidden_size, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                  name='Wmh')
            ...
            Wix2 = tf.get_variable(shape=[self.item_dim, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                   name='Wix2')
            Wih2 = tf.get_variable(shape=[self.hidden_size, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                   name='Wih2')
            ...
            # 遗忘门
            Wfx2 = tf.get_variable(shape=[self.item_dim, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                   name='Wfx2')
            Wfh2 = tf.get_variable(shape=[self.hidden_size, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                   name='Wfh2')
            ...
            # 输出门
            Wox2 = tf.get_variable(shape=[self.item_dim, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                   name='Wox2')
            Woh2 = tf.get_variable(shape=[self.hidden_size, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                   name='Woh2')
            ...
            # 当前记忆
            Wmx2 = tf.get_variable(shape=[self.item_dim, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                   name='Wmx2')
            Wmh2 = tf.get_variable(shape=[self.hidden_size, self.hidden_size], initializer=tf.orthogonal_initializer(),
                                   name='Wmh2')
