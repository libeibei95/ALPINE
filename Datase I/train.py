...
# 下面是视觉特征
visual = "data/visual64_select.npy"
...
def getTrainBatch(batchSize, num):
    ...
    for i in range(num * batchSize, (num + 1) * batchSize):
        ...
        pos_input[i % batchSize] = pos_his_data[user_id][:pos_len] + [len(visual) - 1] * (
                max(timestep, pos_len) - pos_len)
        ...
        neg_input[i % batchSize] = neg_his_data[user_id][:neg_len] + [len(visual) - 1] * (
                max(timestep, neg_len) - neg_len)
    ...
    return label, pos_input, pos_mask_input, pos_edge_input.reshape(
        [-1, timestep, 1]), neg_input, neg_mask_input, neg_edge_input.reshape(
        [-1, timestep, 1]), item_input, user_id_input
...
def eva(sess, model):
    ...
    for valNum in tqdm(range(valLoop)):
        label, pos_input, pos_mask_input, pos_edge_input, neg_input, neg_mask_input, neg_edge_input, \
        item_input, user_id_input = getValBatch(val_batchsize, valNum)
